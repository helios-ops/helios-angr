...
#from .taint_engine_ail import SimEngineRDAIL
from .taint_engine_vex import STAEngineVEX
from .taint_funcHandler import STAFuncHandler 
...
class STAState:
    """
    Represents the internal state of the StaticTaintAnalysis.
    As we make taint propagation during Reaching Definition Analysis, the STAState contains the original STAState 
    defined for ReachingDefinitionAnalysis, which contains definitions and uses for register, stack, memory, and temporary 
    variables, uncovered during the analysis.
    ...
    :param Boolean init_func: Whether or not the internal state of the analysis should be initialized.
                              As for intraprocedural analysis, the callee should be in the analysis context of the caller, 
                              so we should opt this parameter to 'false' in this case.
    ...
    def __init__( self, 
        ...
        self.taint_state = TaintState(self.arch.initial_sp)
    ...
    ## during interprocedural analysis, we should not call this for other functions except the 'main' function.
    def _init_func(self, cc, func_addr):
        ...
        bp = Register(self.arch.sp_offset, self.arch.bytes)
        bp_def = Definition(sp, None, DataSet(self.arch.initial_sp + 0x1000, self.arch.bits))
    ...
    def copy(self):
        ...
        sta_state.register_definitions = self.register_definitions.copy()
        sta_state.stack_definitions = self.stack_definitions.copy()
        sta_state.memory_definitions = self.memory_definitions.copy()
        sta_state.tmp_definitions = self.tmp_definitions.copy()
        sta_state.register_uses = self.register_uses.copy()
        sta_state.stack_uses = self.stack_uses.copy()
        sta_state.memory_uses = self.memory_uses.copy()
        sta_state.tmp_uses = self.tmp_uses.copy()
        sta_state._dead_virgin_definitions = self._dead_virgin_definitions.copy()

        sta_state.taint_state = self.taint_state.copy()
    ...
    ## 在 forward_analysis 框架中，当节点 a 分析结束时，将会更新 a 的每个后继节点 b 的入口点状态。
    ## 这里顺应分析类提供的合并算子 _merge_states，将 a 节点本轮的计算出口状态和 b 节点之前的计算状态进行合并。
    ...
    ## taint state manipulation
    # --------------------------------------------------------------------------- #
    '''
    def update_taintState(self, atom, code_loc, data, dummy = False):
        if type(atom) is Register:
            self.update_register_taintState(atom, code_loc, data, dummy=dummy)
        elif type(atom) is SpOffset:
            self.update_stack_taintState(atom, code_loc, data, dummy=dummy)
        elif type(atom) is MemoryLocation:
            self.update_memory_taintState(atom, code_loc, data, dummy=dummy)
        elif type(atom) is Tmp:
            self.update_tmp_taintState(atom, code_loc, data, dummy=dummy)
        else:
            raise NotImplementedError()


    def update_register_taintState(self, atom, code_loc, data, dummy):
        current_defs = self.register_taintState.get_objects_by_offset(atom.offset)
        if current_defs:
            uses = set()
            for current_def in current_defs:
                uses |= self.stack_uses.get_uses(current_def)
    ...
    def update_memory_taintState(self, atom, code_loc, data, dummy):


    def update_stack_taintState(self, atom, code_loc, data, dummy):

    
    def update_tmp_taintState(self, atom, code_loc, data, dummy):
    '''
    # --------------------------------------------------------------------------- #


class StaticTaintAnalysis(ForwardAnalysis, Analysis):  # pylint:disable=abstract-method
    """
    StaticTaintAnalysis is a text-book implementation of a static data-flow analysis that works on either a
    ...
    * Some more documentation and examples would be nice.
    ...
    def __init__( self, 
                  ...
                  init_state               = None,   ## set for interprocedural analysis
                  init_func                = False,  ## set to 'false' for interprocedural analysis 
                  cc                       = None, 
                  #function_handler         = None,
                  current_local_call_depth = 1, 
        ...
        :param angr.analyses.reaching_definitions.reaching_definitions.STAState init_state:
        ...
        self._function_handler = STAFuncHandler 
        ...
        if type(self) is StaticTaintAnalysis and not self._observe_all and not self._observation_points:
        ...
        self._engine_vex = STAEngineVEX( self.project, 
                                         self._current_local_call_depth, 
    ...
    def return_state(self):
        return_node_set = self._function.endpoints_with_type['return']
        #print ("return_node_set : len = " + str(len(return_node_set)))

        if len(return_node_set) > 0:
            return_node = list(return_node_set)[0]
            return self.get_reaching_definitions_by_node(return_node.addr, OP_AFTER)
        else:
            return None
    ...
    def get_reaching_definitions_by_insn(self, ins_addr, op_type):
        ...
        print (self.observed_results.keys())
    ...
    def node_observe(self, node_addr, state, op_type):
        """
        :param int node_addr:
        :param angr.analyses.reaching_definitions.STAState state:
        ...
        if self._observe_all or \
            ...
            for node in self._function.endpoints_with_type['return']:
    ...
    def insn_observe(self, insn_addr, stmt, block, state, op_type):
        ...
        '''
        outstr = 'insn--' + hex(insn_addr) + "--" + str(op_type)
        print ("observation_point :" + outstr)
        print ("self._observation_points: ")
        print (self._observation_points)
        '''

        if self._observe_all or \
                self._observation_points is not None and key in self._observation_points:
            #print ("GET OBSERVATION POINT: " + str(key) )
    ...
    ## 当分析目标节点没有入口状态的时候，将调用这里的回调函数构造一个抽象的初始状态。
    ## (参见 forward_analysis.py 中的 _analyze 函数）
    def _initial_abstract_state(self, node):
        ## 如果有启动分析时用户提供的初始状态，即用之
        if self._init_state is not None:
            return self._init_state
        else:
            return STAState( self.project.arch, 
                             track_tmps = self._track_tmps, 
                             analysis   = self,
    ...
    ## 在 forward_analysis 框架中，当节点 a 分析结束时，将会更新 a 的每个后继节点 b 的入口点状态。
    ## 这里提供合并算子，将 a 节点本轮的计算出口状态和 b 节点之前的计算状态进行合并。
    def _merge_states(self, node, *states):
        return states[0].merge(*states[1:])

    def _run_on_node(self, node, state):
        #print("run_on_node : addr = " + hex(node.addr) + ", state-id = " + str(id(state)))

        #print ("run_on_node(): type(node) = " + str(type(node)))
        ...
        '''
        if isinstance(node, (Block, CodeNode)):
            block = self.project.factory.block(node.addr, node.size, opt_level=0)
            block_key = node.addr
            engine = self._engine_vex
        else:
        ...
        #-------------------------------#
        print("run_on_mode ---- ")
        print ("handle_read() --- check taint --- taint_buf = " + "0x804c040" + ", size = 4")
        
        taint_dict = state.taint_state.get_memory_taint( 0x804c040,
                                                         4
                                                       )
        ...
        ## TODO: consider why original code contains this
        '''
...
register_analysis(StaticTaintAnalysis, "StaticTaintAnalysis")
