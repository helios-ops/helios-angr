...
from ...engines.engine import SimEngine
...
class STA_SimEngineLight(SimEngine):
    def __init__(self):
        super(STA_SimEngineLight, self).__init__()
...
class STA_SimEngineLightVEXMixin:
    ...
    def _process_Stmt(self, whitelist=None):
        ...
        ## 之于 call 语句，寻找目标地址对应的 handler
        ## 这里注意的是，call 指令在 VEX 中被翻译成 PUT 语句（跳转的类型为 Ijk_CALL）
    ...
    def _handle_WrTmp(self, stmt):
        ...
        if data is None:
            ...
            ## taint propagation
            # --------------------------------------------------------------------- #
            self.state.taint_state.update_tmp_taintState( stmt.tmp,
                                                          size,
                                                          None,
                                                          merge = False
                                                        )
        ...
        self.state.taint_state.update_tmp_taint( stmt.tmp,
                                                 size,
                                                 taint_dict,
                                                 merge = False
                                               )
    ...
    def _handle_RdTmp(self, expr):
        ...
        if tmp in self.tmps:
            ## taint propagation
            # --------------------------------------------------------------------- #
            taint_dict = self.state.taint_state.get_tmp_taint( stmt.tmp,
                                                               size,
                                                             )
        ...
        print ("sta_base_engine: RdTmp: returns None !")
        return None
    ...
    def _handle_Binop(self, expr):
        ...
        # LL: add handle Iop_32HLto64
        elif expr.op.startswith('Iop_32HLto64'):
    ...
    def handle_binop_taint_common(self, expr, arg0, arg1):
        ## taint propagation
        # --------------------------------------------------------------------- #
        bits = expr.result_size(self.tyenv)
        size = bits // self.arch.byte_width

        ts1 = self.state.taint_state.get_expr_taint( id(arg0),
                                                     size 
                                                   )
        ts2 = self.state.taint_state.get_expr_taint( id(arg1),
                                                     size 
                                                   )
    ...
    def _handle_And(self, expr):
        ...
        ## taint propagation
        self.handle_binop_taint_common(expr, arg0, arg1)
    ...
    def _handle_Not(self, expr):
        ...
        bits = expr.result_size(self.tyenv)
        size = bits // self.arch.byte_width
        taint_dict = self.state.taint_state.get_expr_taint( id(arg0), size )
        self.state.taint_state.update_expr_taint( id(expr), size, taint_dict )
        # --------------------------------------------------------------------- #
...
# Compatibility
STA_SimEngineLightVEX = STA_SimEngineLightVEXMixin
#STA_SimEngineLightAIL = STA_SimEngineLightAILMixin
